...
actor HarnessDiscoveryService {
    ...
    func discoverOne(definition: HarnessDefinition, context: ScanContext) async -> HarnessSnapshot {
        ...
        // Desktop bundle versions take precedence over embedded editor CLI versions.
        if context.runVersionCommands, snapshot.installedVersion == nil, let binaryPath = snapshot.binaryPath {
    ...
    nonisolated func findApplication(definition: HarnessDefinition) -> String? {
        ...
        let candidateNames: [String] = {
            ...
            case "t3-code": return ["T3 Code.app", "T3 Code (Alpha).app"]
    ...
    nonisolated private func detectInstallSourceLocally(
        ...
        for method in definition.installationMethods {
            ...
            case .homebrew(let formula):
                if scan.brewFormulas.contains(where: { $0 == formula || $0 == formula.split(separator: "/").last.map(String.init) || $0.hasSuffix("/\(formula)") }) {
                    return .homebrew
                }
